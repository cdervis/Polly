...
for Polly-based games, see [Getting Started](introduction.md#getting-started).
...
Polly uses C++20, with exceptions and RTTI **enabled**.
...
Such checks are out-of-bounds, nullptr-access and empty-optional checks. These can be turned off by `#define`-ing `polly_no_hardening`.
