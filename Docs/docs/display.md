...
Another example would be to manually position and size the game's window within specific display bounds, i.e. by using `#!cpp Window::setPosition()` and `#!cpp Window::setSize()`.
...
To query the displays that are attached to a system, use `#!cpp Game::displays()`.
