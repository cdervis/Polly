...
Polly comes with an embedded font that is directly available via the static `#!cpp Font::builtin()` method. Alternatively, you can drop a font file (.ttf or .otf) into your game's `assets` folder and load it directly.
...
Text is drawn using `#!cpp Painter::drawString()` which takes a `Font`, a size and the string to draw. The string is expected to be UTF-8-encoded.

In your game's `#!cpp draw()` method, simply call `#!cpp drawString()` using the font object and give it some text:
...
**Note**, you can also pass an empty `Font` object, which is the same as using `#!cpp Font::builtin()`.
Example:
...
For cases when text should be highlighted or otherwise hint at certain information, `#!cpp drawString()` provides a way to decorate text, namely using the `TextDecoration` type.
...
![img](../assets/images/drawing_text2.webp){.align-center width=400}

### Text Samplers

!!! note
    This assumes that you've read [Samplers](samplers.md).

It's important to note that the active sampler (set by `#!cpp Painter::setSampler()`) also affects how text is drawn. This is by design, since it allows you to draw pixelated fonts easily. Just set a nearest-neighbor sampler (i.e. `pointClamp`) and then draw your text as usual.
...
For such cases, `Font` provides methods such as `#!cpp measure()`, `#!cpp lineHeight()` and `#!cpp forEachGlyph()`.

## Measuring

We'll first go over the `#!cpp measure()` method. It allows us to get the exact size of a text, in pixels, given a font and a size.
...
Next up is the `#!cpp lineHeight()` method. It gives you the fixed height of the font itself, **in pixels**. This is for example used to align text objects next to each other vertically ("line by line").
