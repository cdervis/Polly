...
This shader declares that it outputs a color with the RGBA components being `#!cpp 0, 1, 0, 1`, meaning it's fully green and fully opaque. The shader's type is specified in the first line (`#!cpp #type sprite`).
...
_For the list of all available variables, see [Shading Language](shading-language.md#built-in-variables)._
...
As you can see, the upper-left corner is black, meaning that a `#!cpp Vec2(0, 0)` was the result.
...
The coordinates range from `#!cpp [0.0 .. 1.0]`, where 0 is the left-most or upper-most pixel and 1 is the right-most or bottom-most pixel location.

So `#!cpp sample(spriteImage, Vec2(0, 0))` would give us the pixel value at location `#!cpp {0 * 32, 0 * 32}`.

On the other hand, sampling at `#!cpp Vec2(1, 1)` would give us the pixel value at location `#!cpp {1 * 32, 1 * 32}`.

Logically, `#!cpp Vec2(0.5, 0.5)` would then be the center pixel value.
...
To implement image tiling, you would simply pass an image coordinate that goes beyond the `#!cpp [0.0 .. 1.0]` range and use a `Sampler` that uses `#!cpp ImageAddressMode::Repeat` or similar. To implement image scrolling, you simply add some offset to your image coordinates.
...
To do that, we use `#!cpp Shader::floatValue()` to obtain the parameter's current value.

**Note** that we must dereference it, because all `#!cpp Shader::...Value()` overloads return a `Maybe`.
...
A safer approach would be to use `#!cpp .valueOr(0.0f)`. In our case, we know that this parameter exists, since we're using it in the shader and therefore it can't be optimized away.

Using `#!cpp set()` doesn't have this problem.

If a parameter name was incorrect or the parameter doesn't exist, the method simply won't do anything.
In other words, it's safe to use `#!cpp set()` even if you're unsure about the parameter's existence.

So now, depending on which key was pressed, we increment / decrement the corresponding intensity by the delta time. Additionally, we clamp the result to the range `#!cpp [0.0 .. 1.0]`.
