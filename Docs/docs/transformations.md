...
You could draw all objects of that scene by offset their positions and scaling their sizes accordingly, for each `#!cpp drawSprite()` call.

Or you could construct a matrix using `#!cpp scale(Vec2(1.5f))` and apply that to all of them.

You can apply a transformation using `#!cpp Painter::setTransformation()`:
...
As an example, `#!cpp Vec2(-1.5f, 2.0f)` will mirror the objects along the Y-axis, and also scale them by a factor of 2.0 along the Y-axis. In other words, negative factors allow mirroring effects.
...
Transformation matrices are often used to implement 2D cameras, since they typically require movement (`#!cpp translate()`) as well as a zoom `#!cpp scale()`).
