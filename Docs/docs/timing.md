...
Animations in your game mainly happen during the `#!cpp Game::update()`. In it, you can use the `GameTime` parameter that is passed to your callback function.
...
This means that on a 60 Hz display the delta would be $1 / 60 \approx 0.1\bar{6}$ seconds, assuming that the system is able to keep up with this frame rate.
