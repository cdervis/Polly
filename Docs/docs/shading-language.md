...
Comments start with `#!cpp //`. Multiline comments in the form of `#!cpp /* ... */` are **not** supported.

## Local Variables

To be consistent with C++, local variables are declared using the `#!cpp auto` keyword. The type of the variable is deduced from its initially assigned value.
...
Use `#!cpp if` statements to conditionally execute a portion of code at runtime:
...
Loops can be realized using a `#!cpp for` statement. A for loop requires a name for the iterator variable and a range in the form of `<start> .. <endExclusive>`:
...
Just like in C++, you can use the `#!cpp continue` keyword to skip a loop iteration, and `#!cpp break` to stop the loop altogether.

## The Main Function

The `#!cpp main()` function is the entry point in a shader.
...
`#!cpp main()` must not have any parameters and must always return a `Vec4` value.
...
Meaning that a `#!cpp float` will be `#!cpp 0.0`, a `Vec2` will be `#!cpp Vec2(0, 0)`, a `Matrix` will be all zeroes, etc.
...
To set parameters on shader objects, call the `#!cpp Shader::set()` method:
...
Conversely, shader parameter values can be obtained using the `#!cpp Shader::...Value()` methods:
...
Array parameter values are also modified using `#!cpp Shader::set()`. Arrays are specified as `Span` values:
...
|--------|------------------------------|----------------|--------------|
| bool   | Boolean true / false value	  | `i32`          | :white_check_mark:            |
| int    | Signed 32-bit integer        | `i32`          | :white_check_mark:            |
| uint   | Unsigned 32-bit integer      | `u32`          | :white_check_mark:            |
| float  | 32-bit floating point number | `float`        | :white_check_mark:            |
| Vec2   | 2D floating point vector     | `Vec2`         | :white_check_mark:            |
| Vec3   | 3D floating point vector     | `Vec3`         | :white_check_mark:            |
| Vec4   | 4D floating point vector     | `Vec4`         | :white_check_mark:            |
| Matrix | 4×4 row-major matrix         | `Matrix`       | :white_check_mark:            |
| Image  | 2D texture                   | `Image`        | :no_entry_sign:            |
