...
The `Window` class represents the game's central window, which can be obtained via `#!cpp Game::window()`:
...
For example, to obtain a window's logical size, use the `#!cpp Window::size()` method, and `#!cpp Window::sizePx()` for its size in pixels.
...
This relation is obtained using the `#!cpp Window::pixelRatio()` method, which represents the DPI scaling factor of the display that currently contains the window. To map from logical units to pixel units, logical values must be multiplied by this factor.
